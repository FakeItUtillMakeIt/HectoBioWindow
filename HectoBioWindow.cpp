    ...
    : QMainWindow(parent)
{
	...
	ui.signalPlot->setAxisScale(QwtPlot::xBottom, 0, sample_freq-(long long int)1, 0);
...
void HectoBioWindow::on_acqparaOK_btn() {
	...
	//一屏显示20s数据
	DISPLAY_PLOT_CNT = sample_freq * 10;
	...
	ui.signalPlot->setAxisScale(QwtPlot::xBottom, 1, (DISPLAY_PLOT_CNT), 0);
	...
	******************************************************/
	...
	else if (ADRun && display_flag)
	{
		...
		if (DISPLAY_MOTHOD0 )
		{
			...
			update_cycle = sample_freq / update_point_length;
		...
		if (!DISPLAY_MOTHOD0)
		{
			...
			double temp = 1000 / update_cycle;
			//先开始线程，在开始计时器
			
			displayUpdateTimer->start(temp);
			display_stop_flag = false;
...
void HectoBioWindow::timerUpdate() {
	...
	if (!DISPLAY_MOTHOD0)
	{
		...
		int sam = update_point_length;
		//数据更新方式1
		//for (int i = 0; i < DISPLAY_PLOT_CNT - sam; i++)
		//{
		...
		//	//每次更新最后一千个点
		//	display_ys[DISPLAY_PLOT_CNT - k] = factDisplayData[displaySample_freq - (update_cycle)*sam - k];
		//}
		//数据更新方式二
		for (int i=0;i<sam;i++)
		{
			display_ys.pop_front();
			display_ys.push_back(factDisplayData[i+sam*(sample_freq/update_point_length-update_cycle)]);
		}
		update_cycle--;
		if (update_cycle == 0)
		{
...
void DisplayThread::run() {
	...
	while (!stop_flag)
	{
		...
		if (stutas) {
			...
			double temp = (displaySample_freq / displaySample_freq);
			double delay_time =1000/temp;
