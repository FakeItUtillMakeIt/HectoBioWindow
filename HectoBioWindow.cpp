	...
	else if (ADRun && display_flag)
	{
		...
		if (!DISPLAY_MOTHOD0)
		{
			...
			if (!USE_TIMER)
			{
				...
				connect(displayUpdateTimer,SIGNAL(timeout()),this,SLOT(threadtimerUpdate()));
			}
...
void HectoBioWindow::on_DA0_btn() {
	...
	float offset_value = ui.da_offset_voltage->text().toDouble();
	offset_value = (offset_value - 21.083) / 0.9923;
			...
			if (display_save_flag && savefile_name != NULL)
			{
				...
				if (display_chn_cnt == 1)
				{
					for (int i = 0; i < displaySample_freq; i++)
					{
						...
						//以下操作是根据实测值和理论值求得的拟合值
						level = (level * (double)1000 + 0.5977) / 0.9854;
						level = (level - 14.819) / 1.0101;
